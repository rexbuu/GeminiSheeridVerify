    ...
    def verify(self) -> Dict:
        ...
        try:
            ...
            self.progress_callback(
                ...
                f"👤 Name: `{first} {last}`\n"
                f"📧 Email: `{email}`\n"
                f"🏫 Studying at: `{self.org['name']}`\n"
                f"🎂 Born: `{dob}` _(totally real)_\n"
                f"🔑 Session: `{self.vid[:16]}...`"
            )
            ...
            if doc_type == "transcript":
                ...
                self.progress_callback("🪪 **Designing ID card...** _adding fake barcode_ 📊")
            ...
            if current_step == "collectStudentPersonalInfo":
                self.progress_callback("📤 **Sending student data...** _knock knock SheerID_ 🚪")
                body = {
                ...
                self.progress_callback("⏭️ **Skipping ahead...** _already past this step_ 🏃")
            else:
                self.progress_callback(f"🤔 **Hmm weird step:** `{current_step}` _YOLO continuing..._")
            
            # Step 3: Skip SSO if needed (PastKing logic)
            if current_step in ["sso", "collectStudentPersonalInfo"]:
                self.progress_callback("� **Bypassing login...** _who needs SSO anyway_ 😏")
                self._request("DELETE", f"/verification/{self.vid}/step/sso")
            
            # Step 4: Upload document
            self.progress_callback("📤 **Uploading masterpiece...** _fingers crossed_ 🤞")
            ...
            self.progress_callback("🏁 **Wrapping up...** _sealing the deal_ 🔐")
            data, status = self._request("POST", f"/verification/{self.vid}/step/completeDocUpload")
            self.progress_callback(f"🎯 **DONE!** Status: `{data.get('currentStep', 'pending')}` _now we wait_ ⏳")
